...
//! The exception to throw when attempting to allocate memory that exceeds a certain limit.
struct TooBigToAllocateException : std::runtime_error {
    //! Constructor.
    explicit TooBigToAllocateException(const char* in_msg) : std::runtime_error(in_msg) {
        ; // Do nothing.
    }

    //! Destructor.
    ~TooBigToAllocateException() override = default;
};
...
public:
    ...
    */
    GenericValue& Reserve(SizeType newCapacity, Allocator &allocator) {
        RAPIDJSON_ASSERT(IsArray());
        if (newCapacity > (std::numeric_limits<unsigned int>::max)() / sizeof(GenericValue)) {
            throw TooBigToAllocateException(
                "Unable to reserve as target capacity exceeded the limit.");
        }
